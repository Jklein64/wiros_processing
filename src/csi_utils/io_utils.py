...
def draw_channel_image(channel):
    ...
    for i in range(num_tx_slots):
        axs = [
            fig.add_subplot(2, num_tx_slots, i + 1),
            fig.add_subplot(2, num_tx_slots, i + num_tx_slots + 1),
        ]
        # plot magnitude
        axs[0].set_xlabel("Subcarrier indices", fontsize=20)
        axs[0].set_ylabel("Magnitude (dB)", fontsize=20)
        for ii in range(channel.shape[1]):
            axs[0].plot(20 * np.log10(np.abs(channel[:, ii, i])), label=f"Rx Ant {ii}")
        ...
        for ii in range(channel.shape[1]):
            axs[1].plot(
                np.unwrap(np.angle(channel[:, ii, i])) * 180 / np.pi,
                label=f"Rx Ant {ii}",
            )
