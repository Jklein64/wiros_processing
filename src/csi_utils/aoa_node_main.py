...
class AoaNode:
    ...
    def draw_prof_image(self, prof):
        if self.use_color:
            profim = (self.accept_color(prof / np.max(prof))[:, :, :3] * 255).astype(
                np.uint8
            )
            peak_idx = int(
                (self._theta_range.shape[0] - 1)
                * (self.last_aoa - self._theta_range[0])
        ...
        axs.set_xticklabels(
            np.linspace(self.d_thresh[0], self.d_thresh[1], self.num_d_steps // 20)
        )
        axs.set_yticks(np.linspace(0, profim.shape[0], self.num_d_steps // 20))
        axs.set_yticklabels(
            np.linspace(
                self.theta_thresh[1], self.theta_thresh[0], self.num_d_steps // 20
            )
        )
    ...
    def export_last_channel(self):
        if (
            self.csi_export_mac_filter is not None
            and self.last_mac != self.csi_export_mac_filter
        ):
            return
    ...
    def csi_callback(self, msg):
        ...
        if self.comp_path is not None:
            if self.use_comp_folder:
                ...
                if comp_spec not in self.comp.keys():
                    spec_file = join(
                        self.comp_path, f"{comp_spec[0]}-{comp_spec[1]}.npy"
                    )
                    if os.path.isfile(spec_file):
                        self.comp[comp_spec] = np.load(spec_file)
                    else:
                        rospy.logerr(
                            f"Tried to load compensation for {comp_spec[0]} on channel {comp_spec[1]}\nbut {spec_file} does not exist. Skipping for now."
                        )
                        self.comp[comp_spec] = 1.0
                self.last_channel = pipeline_utils.extract_csi(
                    msg, self.comp[comp_spec], self.apply_nts, self.valid_tx_ant
                )
            else:
                self.last_channel = pipeline_utils.extract_csi(
                    msg, self.comp, self.apply_nts, self.valid_tx_ant
                )
        else:
            self.last_channel = pipeline_utils.extract_csi(
                msg, None, self.apply_nts, self.valid_tx_ant
            )
        self.last_mac = mac
        ...
        try:
            self.last_aoa, prof = self.aoa_sensors[mac](
                self.last_channel, (msg.chan, bw)
            )
        ...
        if prof is not None and self.pub_prof:
            if self.aoa_sensors[mac].prof_dim == 2:
                ...
                if self.use_color:
                    profim = (
                        self.accept_color(prof / np.max(prof))[:, :, :3] * 255
                    ).astype(np.uint8)
                    peak_idx = int(
                        (self._theta_range.shape[0] - 1)
                        * (self.last_aoa - self._theta_range[0])
        ...
        if self.pub_channel and self.last_channel is not None:
            if self.pub_rel_channel:
                channel_im = io_utils.draw_channel_image(
                    self.last_channel / self.last_channel[:, 0, np.newaxis, :]
                )
            else:
    ...
    def start(self):
        self._theta_range = np.linspace(
            self.theta_thresh[0] * np.pi / 180,
            self.theta_thresh[1] * np.pi / 180,
