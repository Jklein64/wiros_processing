...
class full_svd_aoa_sensor:
    ...
    def __call__(self, H, chanspec):
        ...
        if chanspec not in self.chanspec_seen:
            ...
            self.svd_window[chanspec] = np.zeros(
                (self.rx_pos.shape[0] * constants.n_sub[bw], self.pkt_window),
                dtype=np.complex128,
            )
        ...
        U_csi = (
            np.linalg.svd(self.svd_window[chanspec])[0][:, 0]
            .reshape((constants.n_sub[bw], self.rx_pos.shape[0]), order="F")
            .T
        )
...
class full_music_aoa_sensor:
    ...
    def __call__(self, H, chanspec):
        ...
        prof = self.Theta[chanspec] @ null_Tau.reshape((n_null, n_rx, n_tau)).transpose(
            (1, 0, 2)
        ).reshape((n_rx, n_null * n_tau))
...
class rx_svd_aoa_sensor:
    ...
    def __call__(self, H, chanspec):
        ...
        if chanspec not in self.chanspec_seen:
            ...
            self.chanspec_seen.add(chanspec)
            self.svd_window[chanspec] = np.zeros(
                (constants.n_sub[bw], self.rx_pos.shape[0], self.pkt_window),
                dtype=np.complex128,
            )
            self.svd_roll[chanspec] = 0
...
class music_aoa_sensor_1d:
    ...
    def __call__(self, H, chanspec):
        ...
        if chanspec not in self.chanspec_seen:
            ...
            self.chanspec_seen.add(chanspec)
            self.svd_window[chanspec] = np.zeros(
                (self.pkt_window, self.rx_pos.shape[0], self.rx_pos.shape[0]),
                dtype=np.complex128,
            )
            self.svd_roll[chanspec] = 0
        ...
        prof = (
            np.reciprocal(np.linalg.norm((self.Theta[chanspec] @ U_csi), axis=1) + eps)
            ** 2
        )
...
def spotfi_mat(rx, freqs, theta, d, len_rect, ht_rect):
    ...
    for i in range(1, ht_rect):
        Phi[i, :] = (
            np.exp(-1j * 2 * np.pi * (mean_freq / C_SPEED) * ant_sep * np.sin(theta))
            ** i
        )

    omega_mat = np.repeat(np.exp(-1j * 2 * np.pi * diff_freq * d / C_SPEED), len_rect)
...
class spotfi_sensor:
    ...
    def __call__(self, H, chanspec):
        ...
        if chanspec not in self.chanspec_seen:
            ...
            self.steering_mat[chanspec] = spotfi_mat(
                self.rx_pos, freqs, self.theta_space, self.tof_space, len_rect, ht_rect
            )

        circ_crop = np.zeros(
            (n_tx_ant, n_rx_ant, len_rect, n_sub - len_rect), dtype=np.complex128
        )
        ...
        csi_smooth = np.zeros(
            (ht_rect * len_rect, 2 * n_tx_ant * (n_sub - len_rect))
        ) * (0 + 0j)
        for jj in range(0, n_tx_ant):
            csi_smooth[:, 2 * jj * len_rect : (2 * jj + 1) * len_rect] = circ_crop[
                jj // 2, :-1
            ].reshape(-1, circ_crop[jj // 2, :-1].shape[2])
        ...
        profile = np.reciprocal(
            col_norm(vecs[:, idx].conj().T @ self.steering_mat[chanspec]) ** 2 + EPS
        )
