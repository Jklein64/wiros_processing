...
rx = np.asarray([0, 0, 0, 0.019, -0.019, 0.0, -0.019, 0.019]).reshape((4, 2))
...
S = (
    transforms.aoa_steering_vec(rx, freqs, g_aoa)
    .transpose(0, 2, 1)
...
for tx1 in range(4):
    for tx2 in range(4):
        for tx3 in range(4):
            initc = np.kron(
                np.asarray([1.0, ru_4[tx1], ru_4[tx2], ru_4[tx3]])[:, np.newaxis],
                np.ones((freqs.shape[0], 1)),
            )
            c, l = comp_utils.music_compensation(
                H, g_aoa, initc, rx, freqs, return_loss=True, verbose=True, M=M
            )
            c = c.conj()
...
ax.imshow(
    np.abs(comp_prof),
    origin="lower",
    extent=(d_vals[0], d_vals[-1], theta_vals[0], theta_vals[-1]),
    zorder=-2,
    cmap="jet",
)
